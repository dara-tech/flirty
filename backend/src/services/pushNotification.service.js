      ...
      subscriptions.map(async (subscription) => {
        try {
          ...
          const isGoogleEndpoint =
            subscription.endpoint &&
            subscription.endpoint.includes("google.com");
          ...
          const endpointType = isAppleEndpoint
            ? "Apple/Safari"
            : isGoogleEndpoint
              ? "Google/Chrome"
              : "Other";
          ...
          if (error.statusCode === 410 || error.statusCode === 404) {
            ...
            logger.info(
              `Removed invalid push subscription ${subscription._id} (${error.statusCode})`,
            );
          } else if (error.statusCode === 403) {
            // Forbidden - might be Apple endpoint issue or VAPID key problem
            logger.warn(
              `Push notification forbidden (403) for subscription ${subscription._id}${isAppleEndpoint ? " - Apple endpoint may require different VAPID configuration" : ""}`,
            );
            // Don't remove subscription on 403 - might be temporary
          } else if (error.statusCode === 413) {
            // Payload too large
            logger.warn(
              `Push notification payload too large for subscription ${subscription._id}`,
            );
          } else if (error.statusCode === 400) {
            // Bad request - might be invalid keys or endpoint format
            logger.warn(
              `Bad request (400) for subscription ${subscription._id}${isAppleEndpoint ? " - Check VAPID keys compatibility with Apple" : ""}`,
            );
          } else {
            // Other errors - log full details
            logger.error(
              `Unexpected error (${error.statusCode || "unknown"}) for subscription ${subscription._id}:`,
              errorDetails,
            );
          }
...
export const sendMessageNotification = async (receiverId, messageData) => {
  try {
    ...
    if (messageData.text) {
      ...
      image = Array.isArray(messageData.video)
        ? messageData.video[0]
        : messageData.video;
