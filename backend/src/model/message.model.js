...
const messageSchema = new mongoose.Schema(
  {
    ...
    text: {
      ...
      required: function () {
        const hasImage = Array.isArray(this.image)
          ? this.image.length > 0
          : this.image;
        const hasAudio = Array.isArray(this.audio)
          ? this.audio.length > 0
          : this.audio;
        const hasVideo = Array.isArray(this.video)
          ? this.video.length > 0
          : this.video;
    ...
    // Reply tracking - reference to the message being replied to
    replyTo: {
      type: mongoose.Schema.Types.ObjectId,
      ref: "Message",
    },
...
// Index for reply tracking (to find all replies to a message)
