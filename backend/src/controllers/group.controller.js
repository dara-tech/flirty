...
// ── transferAdmin ─────────────────────────────────────────────────────────────
// PUT /:id/transfer-admin  body: { newAdminId }
export const transferAdmin = async (req, res) => {
  try {
    ...
    const newAdminObjId = new mongoose.Types.ObjectId(newAdminId);
    ...
    if (!isMember) {
      return res
        .status(400)
        .json({ error: "New admin must be a current member" });
    }

    // Move current admin to members, promote newAdmin.
    group.members = group.members.filter(
      (m) => m.toString() !== newAdminId.toString(),
    );
    group.members.push(group.admin);
    group.admin = newAdminObjId;
    await group.save();
...
// ── promoteToAdmin ────────────────────────────────────────────────────────────
// PUT /:id/promote-admin  body: { memberId }
// Adds a co-admin (keeps existing admin, just grants elevated status via a coAdmins array if present, or re-uses the pattern)
export const promoteToAdmin = async (req, res) => {
  try {
    ...
    if (group.admin.toString() !== userId.toString()) {
      return res
        .status(403)
        .json({ error: "Only the admin can promote members" });
    }
    ...
    // Store co-admins in an array field if the schema supports it; otherwise treat as admin transfer.
    if (Array.isArray(group.coAdmins)) {
      const alreadyCoAdmin = group.coAdmins.some(
        (a) => a.toString() === memberId.toString(),
      );
      if (!alreadyCoAdmin) {
        group.coAdmins.push(new mongoose.Types.ObjectId(memberId));
        await group.save();
      }
    }
    ...
    res.status(200).json({ message: "Member promoted", group });
  } catch (error) {
...
// ── demoteFromAdmin ───────────────────────────────────────────────────────────
// PUT /:id/demote-admin  body: { memberId }
export const demoteFromAdmin = async (req, res) => {
  try {
    ...
    if (group.admin.toString() !== userId.toString()) {
      return res
        .status(403)
        .json({ error: "Only the admin can demote members" });
    }

    if (Array.isArray(group.coAdmins)) {
      group.coAdmins = group.coAdmins.filter(
        (a) => a.toString() !== memberId.toString(),
      );
      await group.save();
    }
    ...
    res.status(200).json({ message: "Member demoted", group });
  } catch (error) {
...
// ── batchUpdateMembers ────────────────────────────────────────────────────────
// PUT /:id/batch-update-members  body: { add?: string[], remove?: string[] }
export const batchUpdateMembers = async (req, res) => {
  try {
    ...
    if (group.admin.toString() !== userId.toString()) {
      return res
        .status(403)
        .json({ error: "Only the admin can update members" });
    }

    const currentMemberStrs = group.members.map((m) => m.toString());

    // Add new members (skip duplicates and admin).
    for (const uid of add) {
      const uidStr = uid.toString();
      if (
        uidStr !== group.admin.toString() &&
        !currentMemberStrs.includes(uidStr)
      ) {
        const exists = await User.exists({ _id: uid });
        if (exists) {
          group.members.push(new mongoose.Types.ObjectId(uid));
          currentMemberStrs.push(uidStr);
        }
      }
    }
    ...
    for (const uid of remove) {
      const uidStr = uid.toString();
      if (uidStr !== group.admin.toString()) {
        group.members = group.members.filter((m) => m.toString() !== uidStr);
      }
    }
    ...
    res.status(200).json({ message: "Members updated", group });
  } catch (error) {
...
// ── searchGroups ──────────────────────────────────────────────────────────────
// GET /search?q=&page=&limit=  — search public groups or groups the user belongs to.
