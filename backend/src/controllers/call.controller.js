...
export const getCallHistory = async (req, res) => {
  try {
    ...
    const calls = await Call.find({
      $or: [{ callerId: userIdObjectId }, { receiverId: userIdObjectId }],
    })
  ...
  try {
    ...
    if (calls.length !== callIds.length) {
      return res.status(403).json({
        success: false,
        error:
          "Some calls not found or you don't have permission to delete them",
      });
    }
...
export const getCallStats = async (req, res) => {
  try {
    ...
    const rejected = allCalls.filter(
    ...
    // Count by type
    const voiceCalls = allCalls.filter(
      (call) => call.callType === "voice",
    ).length;
    const videoCalls = allCalls.filter(
      (call) => call.callType === "video",
    ).length;

    // Count by direction
    const outgoing = allCalls.filter(
      (call) => call.callerId.toString() === userId.toString(),
    ).length;
    const incoming = allCalls.filter(
      (call) => call.receiverId.toString() === userId.toString(),
    ).length;
...
/**
 * Reject a call via HTTP API
 *
 * This is a fallback for when socket is not connected (e.g., app was terminated).
 * It handles the same logic as socket.on("call:reject") but via HTTP.
 *
...
export const rejectCall = async (req, res) => {
  try {
    ...
    logger.info(
      `📞 [HTTP Call Reject] User ${userId} rejecting call ${callId}`,
    );

    // Get the io instance from app
    const io = req.app.get("io");
    const { activeCalls, pendingCalls, getReceiverSocketId } =
      req.app.get("callState") || {};

    if (!io || !activeCalls || !pendingCalls) {
      logger.error("Call state not available in app context");
      return res.status(500).json({
        success: false,
        error: "Server not properly configured for call handling",
      });
    }

    // 🔥 Check both activeCalls AND pendingCalls (same as call:reject socket)
    ...
    if (!callInfo) {
      logger.warn(`⚠️ [HTTP Call Reject] Call not found: ${callId}`);
      // Still return success - call may have already ended
    ...
    logger.info(
      `🚫 [HTTP Call Reject] Call ${callId} rejected by ${userId} (reason: ${status})`,
    );
    ...
    try {
      ...
      logger.error("❌ Error saving rejected call record:", saveError);
    }

    // Notify caller with appropriate event based on reason
    const callerSocketId = getReceiverSocketId
      ? getReceiverSocketId(callInfo.callerId)
      : null;
    ...
    if (callerSocketId) {
      ...
      logger.info(`✅ [HTTP Call Reject] Notified caller via socket`);
    }
    ...
    try {
      ...
      logger.info(`✅ [HTTP Call Reject] Sent push notification to caller`);
    } catch (pushError) {
      logger.error("Failed to send call reject push notification:", pushError);
    }

    // Remove call from both maps
    ...
    logger.error("Error in rejectCall", {
      requestId: req?.requestId,
      error: error.message,
      stack: error.stack,
    });
