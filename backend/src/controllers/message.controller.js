...
// ── getPinnedMessages ─────────────────────────────────────────────────────────
// GET /messages/pinned/:id  — returns all pinned messages for a conversation/group.
  ...
  try {
    ...
    const isGroup =
      ...
      !(await Message.exists({
        $or: [{ senderId: conversationId }, { receiverId: conversationId }],
      }));
    ...
    logger.error("Error in getPinnedMessages:", { error: error.message });
    res.status(500).json({ error: "Internal server error" });
  }
};

// ── deleteIndividualMediaItem ─────────────────────────────────────────────────
// DELETE /messages/media-item/:id  — remove a single item from a batch media array.
...
export const deleteIndividualMediaItem = async (req, res) => {
  try {
    ...
    const field = message[mediaType];
    if (Array.isArray(field)) {
      if (index >= field.length) {
        return res.status(400).json({ error: "Index out of range" });
      }
      field.splice(index, 1);
      if (field.length === 0) message[mediaType] = null;
    } else {
      // Single-value field — just clear it.
      message[mediaType] = null;
    }
    ...
    if (message.groupId) {
      ...
      const group = await Group.findById(message.groupId);
      if (group) {
        [group.admin, ...group.members].forEach((memberId) => {
          const sid = getReceiverSocketId(memberId.toString());
          if (sid) io.to(sid).emit("messageEdited", messageObj);
        });
      }
    } else {
      const receiverSocketId = getReceiverSocketId(
    ...
    res.status(200).json({ message: "Media item deleted", data: messageObj });
  } catch (error) {
    logger.error("Error in deleteIndividualMediaItem:", {
      error: error.message,
    });
    res.status(500).json({ error: "Internal server error" });
  }
...
// ── searchMessages ────────────────────────────────────────────────────────────
// GET /messages/search/:conversationId?q=&page=&limit=
export const searchMessages = async (req, res) => {
  try {
    ...
    const baseFilter = mongoose.Types.ObjectId.isValid(conversationId)
      ? {
          $or: [
            { groupId: conversationId },
            {
    ...
    const filter = { ...baseFilter, text: { $regex: regex } };
    ...
    logger.error("Error in searchMessages:", { error: error.message });
    res.status(500).json({ error: "Internal server error" });
  }
};

// ── getMessagesAround ─────────────────────────────────────────────────────────
// GET /messages/around/:conversationId/:messageId?limit=
// Returns messages before + after a specific message (jump-to-message).
export const getMessagesAround = async (req, res) => {
  try {
    const { conversationId, messageId } = req.params;
    const userId = req.user._id;
    const half = Math.floor(parseInt(req.query.limit || 30) / 2);
    ...
    const [before, after] = await Promise.all([
      Message.find({ ...baseFilter, createdAt: { $lte: pivot.createdAt } })
        .sort({ createdAt: -1 })
        .limit(half + 1)
        .populate("senderId", "fullname profilePic")
        .populate("receiverId", "fullname profilePic"),
      Message.find({ ...baseFilter, createdAt: { $gt: pivot.createdAt } })
        .sort({ createdAt: 1 })
        .limit(half)
        .populate("senderId", "fullname profilePic")
        .populate("receiverId", "fullname profilePic"),
    ...
    res.status(200).json({ messages, pivotId: messageId });
  } catch (error) {
    logger.error("Error in getMessagesAround:", { error: error.message });
    res.status(500).json({ error: "Internal server error" });
  }
};
