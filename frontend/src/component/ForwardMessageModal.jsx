...
const ForwardMessageModal = ({ message, onClose }) => {
  const { users, groups, getUsers, getGroups } = useChatStore();
  ...
  const handleForward = async () => {
    ...
    try {
      ...
      if (message._id) {
        messagePayload.forwardedFrom = {
          messageId: message._id,
        };
      }
      
      // Validate that we have at least some content to forward
      if (!messagePayload.text && !messagePayload.image && !messagePayload.video && !messagePayload.audio && !messagePayload.file) {
        toast.error("Message has no content to forward");
        return;
      }

      console.log("Forwarding message payload:", {
        hasText: !!messagePayload.text,
        hasImage: !!messagePayload.image,
        hasVideo: !!messagePayload.video,
        hasAudio: !!messagePayload.audio,
        hasFile: !!messagePayload.file,
        recipients: selectedRecipients.length
      });

      // Forward to each selected recipient
      const forwardPromises = selectedRecipients.map(async (recipient) => {
        try {
          if (recipient.type === "user") {
            return await useChatStore.getState().sendMessageToUser(recipient.id, messagePayload);
          } else if (recipient.type === "group") {
            return await useChatStore.getState().sendGroupMessage(recipient.id, messagePayload);
          }
        } catch (error) {
          console.error(`Error forwarding to ${recipient.type} ${recipient.id}:`, error);
          throw error;
        }
      });
      ...
      toast.error(error.response?.data?.error || error.message || "Failed to forward message");
    } finally {
  ...
  const getRecipientAvatar = (id, type) => {
    if (type === "user") {
      const user = users?.find((u) => u._id === id);
      return user?.profilePic;
    } else {
      const group = groups?.find((g) => g._id === id);
      return group?.profilePic;
    }
  };
    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-base-100 rounded-2xl shadow-2xl w-full max-w-md max-h-[80vh] flex flex-col">
        ...
        <div className="flex items-center justify-between p-4 border-b border-base-200">
          <h2 className="text-lg font-semibold text-base-content">Forward Message</h2>
          <button
            onClick={onClose}
